...
async function acquireForecast(env,place){
 ...
 let data;try{const response=await fetch(forecastSourceURL(place),{signal:AbortSignal.timeout(12000)});if(!response.ok)throw Error();data=await response.json();if(!Number.isFinite(data.current?.temperature_2m)||!Number.isFinite(data.current?.weather_code)||!Array.isArray(data.hourly?.time)||!data.timezone)throw Error()}catch{if(previous)return forecastResponse(env,previous,true);fail(503,'La fonte meteo non risponde. Riprova tra poco.')}
