...
function forecastChanges(before,after,at){
 if((before?.source||'Open-Meteo')!==(after?.source||'Open-Meteo'))return [];
...
async function forecastResponse(env,row,stale=false){
 ...
 const lastChange=last&&(JSON.parse(last.payload).source||'Open-Meteo')===(JSON.parse(row.payload).source||'Open-Meteo')?forecastSummary(last):null;if(lastChange?.previousId){const before=await q(env,'SELECT captured FROM forecast_copies WHERE id=?',lastChange.previousId).first();lastChange.previousCapturedAt=before?.captured||null;}
...
async function acquireForecast(env,place){
 ...
 }catch{if(previous)return forecastResponse(env,previous,true);fail(503,!env.WEATHERAPI_KEY&&Date.now()<forecastRetryAt?'Open-Meteo ha raggiunto il limite di richieste. Le previsioni sono temporaneamente indisponibili.':'La fonte meteo non risponde. Riprova tra poco.')}
 ...
 await q(env,'INSERT OR IGNORE INTO forecast_copies(id,location,slot,captured,timezone,previous_id,changed,changes,payload,hash) VALUES(?,?,?,?,?,?,?,?,?,?)',crypto.randomUUID(),place.key,slot,at,data.timezone,previous&&(JSON.parse(previous.payload).source||'Open-Meteo')===data.source?previous.id:null,Number(changes.length>0),JSON.stringify(changes),payload,hash).run();
 const saved=await q(env,'SELECT * FROM forecast_copies WHERE location=? AND slot=?',place.key,slot).first();if((JSON.parse(saved.payload).source||'Open-Meteo')!==data.source)return unrecorded();return await forecastResponse(env,saved);
...
async function forecastApi(req,env,url){
 ...
 return json({copies:rows.slice(0,20).map(forecastSummary),next:rows.length>20?rows[19].captured:null,source:'Fonti registrate nelle singole copie',modelIssuedAt:null});
 }
