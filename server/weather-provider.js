...
async function weatherProviderCurrent(env,place){
 ...
 try{
  const data=await globeSnapshot(env,'weatherapi-current-v1:'+place.key,async()=>{
   ...
   return {source:'WeatherAPI',sourceURL:'https://www.weatherapi.com/',checkedAt:Date.now(),observedAt:c.last_updated_epoch*1000,timezone:loc.tz_id,
