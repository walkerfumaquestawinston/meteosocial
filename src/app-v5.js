...
(function () {
  ...
  const mkLabel = (text, big) => { const cv = document.createElement('canvas'); const s = big ? 2 : 1.6; cv.width = 512; cv.height = 128; const c = cv.getContext('2d'); let fs = big ? 52 : 44; c.font = `700 ${fs}px Sora, Manrope, sans-serif`; while (c.measureText(text).width > 480 && fs > 26) { fs -= 2; c.font = `700 ${fs}px Sora, Manrope, sans-serif`; } c.textAlign = 'center'; c.lineWidth = 8; c.strokeStyle = 'rgba(0,0,0,.75)'; c.strokeText(text, 256, 84); c.fillStyle = '#fff'; c.fillText(text, 256, 84); const tex = new THREE.CanvasTexture(cv); tex.anisotropy = 4; const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, transparent: true, depthTest: false })); sp.scale.set(0.5 * s / 2, 0.125 * s / 2, 1); return sp; };
