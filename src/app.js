...
(function initMap() {
  ...
  function draw() {
    ...
    WX.forEach((c, i) => {
      ...
      if (layer === 'rain' || layer === 'hail') { const p = Math.max(...c.hours.slice(0, 12).map(h => h[2])); ctx.fillStyle = dark ? 'rgba(255,255,255,.55)' : 'rgba(15,27,46,.45)'; ctx.beginPath(); ctx.arc(x, y, 3.5, 0, 7); ctx.fill(); if (layer === 'rain' && p >= 15 && free(x, y - 9)) { ctx.fillStyle = dark ? '#EAF1FB' : '#0F1B2E'; ctx.strokeStyle = dark ? 'rgba(0,0,0,.6)' : 'rgba(255,255,255,.8)'; ctx.lineWidth = 3; ctx.strokeText(p + '%', x, y - 9); ctx.fillText(p + '%', x, y - 9); } }
