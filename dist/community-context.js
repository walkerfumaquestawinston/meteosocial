import {weatherName} from './weather-tools.js';
...
export function sameSkyTopic(weather) {
  ...
  const observed = Date.parse(weather.current.time+'Z')-Number(weather.utc_offset_seconds||0)*1000;
...
export function communityForecast(weather) {
  ...
  if (!Number.isFinite(current?.temperature_2m)) return {temperature:'—',description:'Previsioni in attesa. Puoi già esplorare i racconti.',source:'Fonte: Open-Meteo'};
  return {
    temperature:Math.round(current.temperature_2m)+'°',
    description:weatherName(current.weather_code),
    source:`${weather._offline?'Dati salvati · ':''}Open-Meteo · ${current.time?.replace('T',' ')||'orario non disponibile'}${weather.timezone?' · '+weather.timezone:''}`
  };
}
