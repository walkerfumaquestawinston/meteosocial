...
export const forecastSource=w=>w?.source==='Rainbow Weather'?'Rainbow Weather':w?.source==='WeatherAPI'?'WeatherAPI':'Open-Meteo';
export const forecastSourceURL=w=>w?.source==='Rainbow Weather'?'https://developer.rainbow.ai/':w?.source==='WeatherAPI'?'https://www.weatherapi.com/':'https://open-meteo.com/';

export const weatherDescription=c=>(typeof c?.condition_text==='string'&&c.condition_text.trim()?c.condition_text.slice(0,100):weatherName(c?.weather_code))+(c?.kind==='forecast'?' · previsione oraria':'');
