...
export function localBrief(weather,state='ok',now=new Date()){
 ...
 const stale=weather._offline||!Number.isFinite(age)||age>120||age < -5||(Number.isFinite(acquired)&&now.getTime()-acquired>90*60000);
...
export function briefMarkup(place,weather,state){
 ...
 return `<div class="map-brief" data-state="${b.status}"><div class="map-brief-location"><span class="map-brief-kicker">IL METEO QUI</span><h2>${esc(place.name)}</h2><span class="map-brief-condition">${esc(b.condition)}</span></div><strong class="map-brief-temperature">${esc(b.temperature)}</strong><div class="map-brief-metrics"><span><small>${esc(b.interval)}</small><b>${esc(b.rain)}</b></span><span><small>Vento medio</small><b>${esc(b.wind)}</b></span><span><small>Open-Meteo</small><b class="map-brief-time">${esc(b.time)}</b></span></div></div>`;
}
export function changeMarkup(weather,state){
 ...
 return `<section class="map-change"><span>IL PROSSIMO CAMBIO</span><strong>${esc(change.title)}</strong>${change.event?`<time>Fascia delle ${esc(change.event.row.time.slice(11,16))}</time>`:''}<p>${esc(change.detail)}</p><small>Previsione Open-Meteo · orari di ${esc(weather.timezone)}</small></section>`;
}
