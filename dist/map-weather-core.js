...
export function weatherPoints(world, towns, readings, selected) {
  const points = towns.map(c => ({name:c[1],country_code:'IT',province:c[2],latitude:c[3],longitude:c[4],
    current:readings.has(c[0]) ? (()=>{const r=readings.get(c[0]);return {temperature_2m:r[1],precipitation:r[2],weather_code:r[3],wind_speed_10m:r[4],interval:r[6],time:r[7]};})() : null}));
  const result = [];
...
export function mapAIRequest(place, question, summary, layer='temperatura') {
  ...
  // the OpenAI payload. No authors, posts, media or history are added here.
