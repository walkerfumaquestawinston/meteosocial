...
export function createMappaEventi(ctx) {
  ...
  function renderRadar(state){
    ...
    const isHail=radarKind==='hail',isRainbow=!isHail&&state.source==='Rainbow Weather';
    $('#mappa-radar-heading').textContent=isHail?'GRANDINE · ITALIA':isRainbow?'PIOGGIA · '+(state.time>state.issuedAt?'PREVISIONE':'ANALISI'):'RADAR PIOGGIA';
    const source=$('#mappa-radar-source');source.textContent=isHail?'Radar-DPC · CC BY-SA ↗':isRainbow?'Rainbow Weather ↗':'RainViewer ↗';source.href=isHail?'https://dpc-radar.readthedocs.io/it/latest/':isRainbow?'https://developer.rainbow.ai/':'https://www.rainviewer.com/';
    $('#mappa-radar-explanation').innerHTML=isHail?'<span class="radar-note-full">POH: probabilità stimata da radar, non grandine confermata. Italia, copertura non uniforme. Trasparente: valore sotto 10% oppure dato assente. Non indica zone sicure.</span><span class="radar-note-mobile">Stima POH · Italia. Trasparente ≠ assenza di grandine.</span>':isRainbow?'Analisi e previsione globale fino a +4 ore. Passo 10 min; base '+clock(state.issuedAt*1000)+'. <a href="https://doc.rainbow.ai/tile_colors/" target="_blank" rel="noopener">Legenda colori Rainbow ↗</a>. Zone vuote non certificano assenza di pioggia. Non identifica grandine.':(state.fallback?'Rainbow non disponibile: mostro RainViewer. ':'')+'Immagini recenti, non previsioni. Copertura variabile: una zona vuota non esclude pioggia.';
    $('#radar-periods').hidden=!isRainbow;
    const legend=$('#radar-color-key'),legendKind=isRainbow?'rainbow':isHail?'hail':'rainviewer';
    if(legend.dataset.kind!==legendKind){legend.dataset.kind=legendKind;legend.innerHTML=isRainbow?'<figure><figcaption>Pioggia · scala ufficiale Rainbow</figcaption><img loading="lazy" src="https://doc.rainbow.ai/images/palletes/rainbow_rain.png" alt="Scala colori della pioggia Rainbow; valori e unità nella legenda ufficiale collegata sotto"></figure><figure><figcaption>Neve · scala ufficiale Rainbow</figcaption><img loading="lazy" src="https://doc.rainbow.ai/images/palletes/rainbow_snow.png" alt="Scala colori della neve Rainbow; valori e unità nella legenda ufficiale collegata sotto"></figure>':isHail?'':'<p class="field-note">Colori RainViewer: <a href="https://www.rainviewer.com/api/color-schemes.html" target="_blank" rel="noopener">apri la scala del fornitore ↗</a></p>';}
    ...
    const time=state.time?clock(state.time*1000):'—';$('#mappa-radar-time').textContent=time;range.setAttribute('aria-valuetext',time+(isRainbow&&state.time>state.issuedAt?' · previsione':' · quadro precipitazioni'));renderFreshness();$('#mappa-radar-latest').disabled=!state.frames.length||state.index===(state.nowIndex??state.frames.length-1);$('#mappa-radar-latest').textContent=isRainbow?'Adesso':'Ultimo';$('#mappa-radar-latest').setAttribute('aria-label',isRainbow?'Torna al quadro di base':'Torna all’ultimo quadro radar');
  ...
  function toolsPanel(){
    ...
    const actionMarkup=ids=>actions.filter(a=>ids.includes(a[0])).map(([id,icon,title,detail])=>`<button data-map-action="${id}">${mapIcon(icon)}<span><b>${title}</b><small>${detail}</small></span></button>`).join('');
    panel('Strumenti della mappa',`<section class="map-tool-section"><h3>Vista</h3><label class="map-appearance">Aspetto della mappa<select id="map-appearance">${[['auto','Automatico · giorno e notte'],['giorno','Sempre chiara'],['notte','Sempre scura']].map(([v,t])=>`<option value="${v}" ${mapAppearance===v?'selected':''}>${t}</option>`).join('')}</select></label><p class="field-note">Automatico segue alba e tramonto della località. Senza orari solari usa 07–19 nel suo fuso.</p><div class="map-tool-grid">${actionMarkup(['mappa-posizione','mappa-zona','mappa-city-names','mappa-mondo'])}</div></section><section class="map-tool-section"><h3>Dati</h3><div class="map-tool-grid"><button id="map-directory-radar">${mapIcon('radar')}<span><b>Pioggia e neve</b><small>Rainbow: analisi e previsione fino a +4 ore</small></span></button>${actionMarkup(['local-details','mappa-elenco','mappa-aggiorna','mappa-eventi'])}</div><p class="field-note">Grandine: scegli il livello in alto. Radar POH della Protezione Civile in Italia; segnalazioni delle persone separate.</p></section><section class="map-tool-section"><h3>Aiuto</h3><div class="map-tool-grid">${actionMarkup(['mappa-guida'])}</div></section>`);
  ...
  function contextSummary(){
    ...
    if(radarState.enabled)info.push(`Radar ${radarKind==='hail'?'Radar-DPC POH Italia':radarState.source||'RainViewer'}: ${radarState.status}, quadro ${radarState.time?new Date(radarState.time*1000).toISOString():'non disponibile'}. Pixel non forniti all’IA: non dedurre intensità, movimento o arrivi dalle immagini.`);
  ...
  async function bind(){
    ...
    $('#mappa-radar-latest').onclick=()=>activeRadar()?.seek(radarState.nowIndex??radarState.frames.length-1);
    $('#mappa-radar-toggle').onclick=()=>chooseRadar(mode==='grandine'?'hail':'rain',!radarState.enabled);
    $('#mappa-radar-fallback').onclick=()=>radar?.useFallback();
