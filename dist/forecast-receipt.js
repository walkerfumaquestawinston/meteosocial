...
export function forecastReceipt(weather){if(!weather)return '';const p=weather._provenance,last=p?.lastChange,change=last?.changes?.find(c=>c.fields.includes('weather_code'))||last?.changes?.[0];return `<div class="forecast-receipt" aria-live="polite"><p>${esc(forecastStamp(weather))}${weather._offline?' · ultimo dato salvato':''}</p>${change?`<details class="forecast-difference"><summary>Ultima variazione della previsione</summary><p>Cambiamento rilevato ${esc(acquiredLabel(last.capturedAt,last.timezone))}: per il ${esc(validHour(change.time))}, ${esc(changeText(change))}.</p></details>`:''}${p?.status==='unavailable'?'<p>Cronologia online non disponibile per questa copia.</p>':''}<button type="button" data-forecast-history>Ora e cronologia</button></div>`}
