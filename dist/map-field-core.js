...
export function fieldHours(data,now=Date.now()){
 ...
 return h.time.map((time,i)=>({time:epoch(time),...Object.fromEntries(['temperature_2m','apparent_temperature','precipitation','precipitation_probability','snowfall','snow_depth','freezing_level_height','visibility','wind_speed_10m','wind_gusts_10m','weather_code','is_day'].map(k=>[k,Number.isFinite(h[k]?.[i])?h[k][i]:null]))})).filter(p=>Number.isFinite(p.time)&&p.time>=now&&p.time<now+7*3600000).slice(0,6);
}
